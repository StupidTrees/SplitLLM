...
class GPT2SplitModel(GPT2Model, SplitModel):
    ...
    # def split(self, sp1=1, sp2=11):
    #     trunk_block = self.h[sp1:sp2]
    #     h1 = self.h[:sp1]
    #     h2 = self.h[sp2:]
    #     cfg = self.config
    #     cfg.n_layer = len(trunk_block)
    #     self.trunk_model = GPT2TrunkModel(cfg, sp1, trunk_block)
    #     self.h = ModuleList(h1 + h2)
    #     self.sp1 = sp1
    #     self.sp2 = sp2
    #     self.already_split = True
    #     print(f'split, end_size:{len(self.h)}, trunk_size:{len(self.trunk_model.h)}')
    #
    # def reset(self):
    #     self.h = ModuleList(self.h[:self.sp1] + list(self.trunk_model.h.modules()) + self.h[self.sp2:])
    #     self.sp1 = 0
    #     self.sp2 = len(self.h)
    ...
    #     if past_key_values is None:
    #         past_length = 0
    #         past_key_values = tuple([None] * self.total_blocks)  # len(self.h))
    #     else:
    #         past_length = past_key_values[0][0].size(-2)
    #     if position_ids is None:
    #         position_ids = torch.arange(past_length, input_shape[-1] + past_length, dtype=torch.long, device=device)
    ...
    #     # attention_probs has shape bsz x n_heads x N x N
    #     # head_mask has shape n_layer x batch x n_heads x N x N
    #     head_mask = self.get_head_mask(head_mask, self.total_blocks)
    #
    #     if inputs_embeds is None:
    #         inputs_embeds = self.wte(input_ids)
    #     position_embeds = self.wpe(position_ids)
    ...
    #     all_cross_attentions = () if output_attentions and self.config.add_cross_attention else None
    #     all_hidden_states = () if output_hidden_states else None
    #     for glob_idx in range(self.total_blocks):
    #         layer_past = past_key_values[glob_idx]
    #         i = glob_idx
    #         if self.sp1 <= glob_idx <= self.sp2:  # trunk-layers:
    #             hidden_states, presents_, all_hidden_states_, all_cross_attentions_, all_hidden_states_ = self.trunk_model.trunk_forward(
    #                 hidden_states,
    #                 attention_mask,
    #                 past_key_values, head_mask, output_hidden_states,
    ...
    #             if output_attentions:
    #                 all_cross_attentions += all_cross_attentions_
    #             continue
    #         elif glob_idx >= self.sp2:  # top-layers
    #             trunk_size = len(self.trunk_model.h)
    #             i -= trunk_size
    #         block = self.h[i]
    #
    #         # Model parallel
    ...
    #         if self.model_parallel:
    #             for k, v in self.device_map.items():
    #                 if glob_idx == v[-1] and "cuda:" + str(k) != self.last_device:
    #                     hidden_states = hidden_states.to("cuda:" + str(k + 1))
    #
    #     hidden_states = self.ln_f(hidden_states)
    #
...
#
# class GPT2TrunkModel(GPT2Model):
#
#     def __init__(self, config, split_point1, trunk_blocks):
#         super().__init__(config)
#         self.h = ModuleList(trunk_blocks)
#         self.split_point = split_point1
#
#     def trunk_forward(self, hidden_states, attention_mask, past_key_values, head_mask, output_hidden_states,
#                       output_attentions, encoder_hidden_states,
#                       encoder_attention_mask, use_cache: Optional[bool] = None, ):
#         presents = () if use_cache else None
#         all_self_attentions = () if output_attentions else None
#         all_cross_attentions = () if output_attentions and self.config.add_cross_attention else None
#         all_hidden_states = () if output_hidden_states else None
#         for idx, block in enumerate(self.h):
#             global_idx = idx + self.split_point
...
#                     layer_past=layer_past,
#                     attention_mask=attention_mask,
#                     head_mask=head_mask[global_idx],
#                     encoder_hidden_states=encoder_hidden_states,
#                     encoder_attention_mask=encoder_attention_mask,
#                     use_cache=use_cache,
#                     output_attentions=output_attentions,
...
#             # if self.model_parallel:
#             #     for k, v in self.device_map.items():
#             #         if global_idx == v[-1] and "cuda:" + str(k) != self.last_device:
#             #             hidden_states = hidden_states.to("cuda:" + str(k + 1))
#         return hidden_states, presents, all_hidden_states, all_cross_attentions, all_hidden_states
