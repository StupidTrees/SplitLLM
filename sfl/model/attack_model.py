...
from transformers import PretrainedConfig, GPT2Config, LlamaConfig, PreTrainedModel
...
class LLAMA2AttackModel(AttackModel):

    def __init__(self, config: LlamaConfig, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
        self.hidden_size = 128
        self.lstm = nn.LSTM(input_size=config.hidden_size, hidden_size=self.hidden_size, batch_first=True)
        self.mlp = nn.Linear(self.hidden_size, config.vocab_size)
