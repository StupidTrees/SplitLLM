...
def get_model_path(model_name):
    ...
    if model_name.startswith('gpt2'):
        ...
        path = os.path.join(model_download_dir, f"{model_name}-uncased/")
    elif model_name.startswith('roberta'):
        path = os.path.join(model_download_dir, f"{model_name}/")
    elif model_name.startswith('flan-t5'):
        path = os.path.join(model_download_dir, f"{model_name}")
    elif model_name.startswith('flan-ul2'):
        path = os.path.join(model_download_dir, f"{model_name}")
    elif model_name.startswith('llama2'):
        path = os.path.join(model_download_dir, f"Llama-2-7b-chat-hf/Llama-2-7b-chat-hf")
    elif model_name.startswith('chatglm'):
        path = os.path.join(model_download_dir, f"chatglm3-6b/chatglm3-6b")
    return path
