...
sip_inverter_datasets=("gsm8k" "wikitext" "piqa" "codealpaca" "sensimarked" "sensimasked" "sensireplaced")
  ...
  for model_name in "${model_names[@]}"; do
    for sip_inverter_dataset in "${sip_inverter_datasets[@]}"; do
      for sfl_dataset in "${sfl_datasets[@]}"; do
        case_name="SD${seed}-SIP@${sfl_dataset}<<${sip_inverter_dataset}"
        # 先训练攻击模型
